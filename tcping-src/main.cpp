...
int main(int argc, char* argv[]) {
	...
	for (int x = 0; x < argc; x++) {
		...
		if (!strcmp(argv[x], "--proxy-server")) {
			snprintf(proxy_server, sizeof(proxy_server), argv[x + 1]);
			offset = x + 1;
		}

		if (!strcmp(argv[x], "--proxy-credentials")) {
			snprintf(proxy_credentials, sizeof(proxy_credentials), argv[x + 1]);
			using_credentials = 1;
			offset = x + 1;
		}
		...
		if (!strcmp(argv[x], "--tee")) {
			strncpy(logfile,static_cast<const char*>(argv[x + 1]),sizeof(logfile));
			offset = x + 1;
			use_logfile = 1;
			show_arg_header = 1;
		}
    ...
    if (argc >= 2 + offset) {
		if (!reading_from_file) {
			...
			strncpy(urlfile, static_cast<const char*>(argv[offset + 1]),sizeof(urlfile));
		}
    ...
    if (use_http == 1 && reading_from_file == 0) {   //added reading from file because if we are doing multiple http this message is just spam.
        ...
        if (docptr != NULL) {
            *docptr = '\0';
			...
			strncpy(server,static_cast<const char*>(serverptr),sizeof(server));
			strncpy(document,static_cast<const char*>(docptr),sizeof(document));
        } else {
			strncpy(server,static_cast<const char*>(serverptr),sizeof(server));
            document[0] = '\0';
        }
